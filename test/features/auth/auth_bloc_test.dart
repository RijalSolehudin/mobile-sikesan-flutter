...
void main() {
  group('AuthBloc Unit Tests', () {
    ...
    test('AuthCheckRequested emits unauthenticated when no token is present', () async {
    ...
    test('AuthCheckRequested emits authenticated when token and user exist', () async {
    ...
    test('AuthLoginRequested emits loading then authenticated on success', () async {
      const user = UserModel(
        id: 1,
        name: 'Kasir Sikesan',
        username: 'kasir_1',
      ...
      authBloc.add(const AuthLoginRequested(username: 'kasir_1', password: '123'));
    ...
    test('AuthLoginRequested emits loading then failure on ApiFailure', () async {
      mockRepo.loginResult = const ApiFailure('Username atau password salah', statusCode: 401);

      authBloc.add(const AuthLoginRequested(username: 'wrong', password: 'bad'));

      await expectLater(
        authBloc.stream,
        emitsInOrder([
          const AuthState(status: AuthStatus.loading),
          const AuthState(status: AuthStatus.failure, errorMessage: 'Username atau password salah'),
        ]),
      );
    });
