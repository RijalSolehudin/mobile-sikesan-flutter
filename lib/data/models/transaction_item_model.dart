class TransactionItemModel {
  ...
  factory TransactionItemModel.fromJson(Map<String, dynamic> json) {
    ...
    final bool isCredit = (type == 'CREDIT' || type == 'TOP_UP' || type == 'INCOME');
    ...
    return TransactionItemModel(
      ...
      title: json['description']?.toString() ?? (isCredit ? 'Top Up Saldo' : 'Pengeluaran Saldo'),
      category: json['reference_type']?.toString() ?? (isCredit ? 'Pemasukan Saldo' : 'Pengeluaran Saldo'),
      amount: (json['amount'] as num?) ?? 0,
      isIncome: isCredit,
      date: json['created_at'] != null ? DateTime.tryParse(json['created_at'].toString()) ?? DateTime.now() : DateTime.now(),
      studentName: studentName,
      studentClass: json['student_class']?.toString() ?? '',
    );
  }

  factory TransactionItemModel.fromSppJson(Map<String, dynamic> json, {bool isGuardian = true}) {
    String studentName = 'Santri';
    if (json['student'] != null && json['student'] is Map) {
      studentName = json['student']['name']?.toString() ?? 'Santri';
    }

    final amount = (json['total_paid_amount'] as num?) ?? (json['amount'] as num?) ?? 0;
    ...
    return TransactionItemModel(
      ...
      amount: amount,
      isIncome: !isGuardian, // Bagi wali santri ini pengeluaran, bagi bendahara penerimaan
  ...
  factory TransactionItemModel.fromInfaqJson(Map<String, dynamic> json, {bool isGuardian = true}) {
    String studentName = 'Santri';
    if (json['student'] != null && json['student'] is Map) {
      studentName = json['student']['name']?.toString() ?? 'Santri';
    }
